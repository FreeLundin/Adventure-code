...
- **Only use official Unreal plugins** in production; experimental or third‑party tools can introduce compatibility issues. For example, rather than relying on an Omniverse connector, use Epic’s USD plugin or built‑in Sequencer features when possible. This reduces upgrade risk and ensures support from Epic.  We now keep a whitelist file and CI check to enforce this policy.
